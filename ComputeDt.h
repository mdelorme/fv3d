    ...
    Kokkos::parallel_reduce("Computing DT",
                            ...
                            KOKKOS_LAMBDA(int i, int j, int k, real_t &inv_dt_hyp, real_t &inv_dt_par_tc, real_t &inv_dt_par_visc) {
                              ...
                              if (params.thermal_conductivity_active)
                                inv_dt_par_tc_loc = fmax(fmax(2.0*computeKappa(i, j, k, params) / (params.dx*params.dx),
                                                              2.0*computeKappa(i, j, k, params) / (params.dy*params.dy)),
                                                              2.0*computeKappa(i, j, k, params) / (params.dz*params.dz));
                              
                              real_t inv_dt_par_visc_loc = params.epsilon;
                              if (params.viscosity_active)
                                inv_dt_par_visc_loc = fmax(fmax(2.0*computeMu(i, j, k, params) / (params.dx*params.dx),
                                                                2.0*computeMu(i, j, k, params) / (params.dy*params.dy)),
                                                                2.0*computeMu(i, j, k, params) / (params.dz*params.dz));
