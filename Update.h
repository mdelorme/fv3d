    ...
    Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const IFace face, const int i, const int j, const int k) {
        for (int ivar=0; ivar < Nfields; ++ivar) {
          ...
          real_t dR = Q(face, k, j, i+1, ivar) - Q(face, k, j, i, ivar);
          real_t dU = Q(face, k, j, i, ivar)   - Q(face, k, j-1, i, ivar); 
          real_t dD = Q(face, k, j+1, i, ivar) - Q(face, k, j, i, ivar); 
          real_t dF = Q(face, k+1, j, i, ivar) - Q(face, k, j, i, ivar);
          real_t dB = Q(face, k, j, i, ivar)   - Q(face, k-1, j, i, ivar);
          ...
          slopesX(face, k, j, i, ivar) = minmod(dL, dR);
          slopesY(face, k, j, i, ivar) = minmod(dU, dD);
          slopesZ(face, k, j, i, ivar) = minmod(dF, dB);
        }
      });
    ...
    Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const IFace face, const int i, const int j, const int k) {
        ...
        auto updateAlongDir = [&](int i, int j, int k, IDir dir) {
          ...
          dm[dir] = -1;
          dp[dir] =  1;
          State qCL = reconstruct(Q, slopes, face, i, j, k, -1.0, dir, params);
          State qCR = reconstruct(Q, slopes, face, i, j, k,  1.0, dir, params);
          State qL  = reconstruct(Q, slopes, face, i+dm[IX], j+dm[IY], k+dm[IZ],  1.0, dir, params);
          State qR  = reconstruct(Q, slopes, face, i+dp[IX], j+dp[IY], k+dp[IZ], -1.0, dir, params);
          ...
          if (params.well_balanced_flux_at_z_bc && (k==params.kbeg || k==params.kend-1) && dir == IZ) {
            if (k==params.kbeg)
              fluxL = State{0.0, 0.0, 0.0, poutR - Q(face, k, j, i, IR)*params.g*params.dz, 0.0};
            else 
              fluxR = State{0.0, 0.0, 0.0, poutL + Q(face, k, j, i, IR)*params.g*params.dz, 0.0};
          }
        ...
        // Unew(face, k, j, i, IR) = fmax(1.0e-6, Unew(face, k, j, i, IR));
      });
  }
